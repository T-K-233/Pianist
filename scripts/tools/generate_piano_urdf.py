# Modified from the RoboPianist codebase
...
def build(add_actuators: bool = False) -> str:
    ...
    urdf_lines = [
        ...
        '  <link name="body">',
        '    <visual>',
        f'      <origin xyz="{consts.BASE_POS[0]} {consts.BASE_POS[1]} {consts.BASE_POS[2]}" rpy="0 0 0" />',
        '      <geometry>',
        f'        <box size="{consts.BASE_SIZE[0]*2} {consts.BASE_SIZE[1]*2} {consts.BASE_SIZE[2]*2}" />',
        '      </geometry>',
        '    </visual>',
        '    <collision>',
        f'      <origin xyz="{consts.BASE_POS[0]} {consts.BASE_POS[1]} {consts.BASE_POS[2]}" rpy="0 0 0" />',
    ...
    # Calculate joint origin (at the back of the key)
    joint_origin_x = consts.BLACK_KEY_X_OFFSET - consts.BLACK_KEY_LENGTH / 2
    joint_origin_y = y_coord
    joint_origin_z = consts.BLACK_KEY_Z_OFFSET

    # Calculate visual/collision origin (center of key)
    visual_origin_x = consts.BLACK_KEY_LENGTH / 2
    visual_origin_y = 0
    visual_origin_z = 0
    ...
    n = 0
    for twin_index in consts.TWIN_INDICES:
        for j in range(2):
            y_coord = (
                -consts.PIANO_LENGTH * 0.5
    ...
    for triplet_index in consts.TRIPLET_INDICES:
        for j in range(3):
            y_coord = (
                -consts.PIANO_LENGTH * 0.5
                + (j + 1) * (consts.WHITE_KEY_WIDTH + consts.SPACING_BETWEEN_WHITE_KEYS)
...
if __name__ == "__main__":
    ...
    save_path = Path("./source/pianist/data/assets/piano/urdf/piano.urdf")
